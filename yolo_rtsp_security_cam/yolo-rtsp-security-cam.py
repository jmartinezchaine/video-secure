...
parser.add_argument('--monitor', default=False, action="store_true", help="View the live stream. If no monitor is connected then leave this disabled (no Raspberry Pi SSH sessions).")
...
parser.add_argument("--threshold", default=2000, type=int, choices=range(1,10000), help="Determines the amount of motion required to start recording. Higher values decrease sensitivity to help reduce false positives. Default 350, max 10000.")
...
parser.add_argument("--auto_delete", default=False, action="store_true", help="Enables auto-delete feature. Recordings that have total length equal to the tail_length value (seconds) are assumed to be false positives and are auto-deleted.")
parser.add_argument('--testing', default=False, action="store_true", help="Testing mode disables recordings and prints out the motion value for each frame if greater than threshold. Helps fine tune the threshold value.")
parser.add_argument('--frame_click', default=False, action="store_true", help="Allows user to advance frames one by one by pressing any key. For use with testing mode on video files, not live streams, so set a video file instead of an RTSP address for the --stream argument.")
...
def start_ffmpeg():
    #ffmpeg_copy.execute()
    print("No hacemos nada por ahora, la idea es grabar el video y guardarlo")
...
chat.mandarMensaje('Yolo esta viendo si encuentra:' + ' '.join(yolo_list), None)
                ...
                if activity_count >= start_frames:
                    ...
                    if not yolo_on or yolo_count > 1:
                        ...
                        if not testing:
                            ...
                            chat.mandarMensaje(telegram_mensaje, imagen_path)
